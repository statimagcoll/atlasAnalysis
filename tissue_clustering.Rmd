...
# might not be the best approach. Would be interested just to see smoothing on raw images
gaussSmooth = function(img, outimg, mask, maskThr, sigma=5, transform=log10, invtransform=function(x) 10^x, offset=1/2, divmean = function(img){ cellStats(img, stat = 'mean')} ){
  ...
  values(ans) = transform(values(ans)/divmean + offset)
  if(sigma>0){
...
for(marker in c(locationMarkers)){
  message(marker)
  invisible(mcmapply(gaussSmooth, sr[ss,marker], sr[ss,paste(marker, 'smooth', sep="_")], sr[ss,'mask'], MoreArgs=list(maskThr=dapiThr, sigma=sigma, transform=function(x) x, invtransform = function(x) x, offset=0), mc.cores = 6 ))
}
